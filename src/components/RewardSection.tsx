...
export const RewardSection: React.FC<Props> = ({
   ...
   */
  const pinnedReward =
    rewards.find((r) => r.id === pinnedRewardId) || rewards[0];

  const pinnedIsReady = pinnedReward
    ? currentPoints >= pinnedReward.requiredPoints
    : false;

  const pinnedRemaining = pinnedReward
    ? Math.max(
        0,
        pinnedReward.requiredPoints - currentPoints
      )
    : 0;
  ...
  const pinnedPercent = pinnedReward
    ? Math.min(
        100,
        Math.round(
          (currentPoints / pinnedReward.requiredPoints) * 100
        )
      )
    : 0;
  ...
  return (
    ...
    >
      ...
      {pinnedReward && (
        <div
          className={`relative overflow-hidden rounded-3xl p-5 border-2 shadow-lg transition-all ${
            pinnedIsReady
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-white/80 line-clamp-1 mt-0.5">
                  {pinnedReward.description ||
                    'پاداش انگیزشی هدف‌گذاری شده'}
                </p>
                ...
                <div className="flex items-center gap-2 mt-2 text-xs font-bold text-yellow-300">
                  ...
                  <span>
                    موجودی فعلی:{' '}
                    {toPersianDigits(currentPoints)} ستاره
                  </span>
            ...
            <div className="pt-1 flex items-center gap-2">
              ...
              {pinnedIsReady ? (
                ...
                isParentMode ? (
                  ...
                  <div className="w-full py-2.5 px-3 bg-white/10 border border-white/10 rounded-xl text-center text-xs font-bold text-white/90 flex items-center justify-center gap-2">

                    <Lock className="w-3.5 h-3.5 text-yellow-300" />

                    <span>
                      این پاداش آماده دریافت است؛
                      از والدین بخواهید آن را تحویل دهند 🎁
                    </span>
                ...
                <div className="w-full py-2 px-3 bg-white/10 rounded-xl text-center text-xs font-medium text-white/80 flex items-center justify-center gap-2">
                  ...
                  <span>
                    با انجام کارهای خوب روزانه،{' '}
                    {toPersianDigits(pinnedRemaining)}{' '}
                    ستاره دیگر کسب کن!
                  </span>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-1">
        ...
        <div>
          ...
          <h2 className="text-base sm:text-lg font-black text-slate-800 flex items-center gap-2">
            ...
            <span>
              فهرست پاداش‌ها و جوایز انگیزشی
            </span>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            ...
            {isParentMode
              ? 'پاداش‌های مختلف را تعریف کنید تا کودک برای دستیابی به آن‌ها تشویق شود'
              : 'پاداش‌های تعریف‌شده و میزان پیشرفت خود را مشاهده کن'}
              ...
              {isParentMode && (
                <div className="absolute left-3 top-3 flex items-center gap-1 z-10">
                  ...
                  <button
                    ...
                    title={
                      isPinned
                        ? 'هدف اصلی انتخاب شده است'
                        : 'تنظیم به عنوان هدف اصلی'
                    }
                  >
              ...
              <div className="mt-4">
                ...
                {isEligible ? (
                  ...
                  isParentMode ? (
                    ...
                    <div className="w-full py-2.5 px-3 bg-emerald-50 border border-emerald-200 rounded-xl text-center text-xs font-bold text-emerald-700 flex items-center justify-center gap-1.5">

                      <Lock className="w-3.5 h-3.5 text-emerald-500" />

                      <span>
                        آماده دریافت از والدین 🎁
                      </span>
      ...
      {filteredRewards.length === 0 && (
        <div className="py-10 text-center bg-white border border-dashed border-slate-300 rounded-3xl">

          <Gift className="w-10 h-10 mx-auto text-slate-300 mb-3" />

          <p className="text-sm font-bold text-slate-500">
            پاداشی در این بخش وجود ندارد.
          </p>

          {isParentMode && (
            <button
              onClick={onAddNewReward}
              className="mt-4 inline-flex items-center gap-2 px-4 py-2.5 bg-pink-500 hover:bg-pink-600 text-white text-xs font-bold rounded-xl transition"
            >
              <Plus className="w-4 h-4" />
              تعریف پاداش جدید
            </button>
